...
impl IMemoryStorage for MockStorage {
    ...
    fn create_bulk(&self, m: &[BaseMemory]) -> CortexResult<usize> { Ok(m.len()) }
    fn get_bulk(&self, _: &[String]) -> CortexResult<Vec<BaseMemory>> { Ok(vec![]) }
    fn query_by_type(&self, _: cortex_core::MemoryType) -> CortexResult<Vec<BaseMemory>> { Ok(vec![]) }
    fn query_by_importance(&self, _: cortex_core::Importance) -> CortexResult<Vec<BaseMemory>> { Ok(vec![]) }
    ...
    fn count_by_type(&self) -> CortexResult<Vec<(cortex_core::MemoryType, usize)>> { Ok(vec![]) }
...
impl IEmbeddingProvider for MockEmbedding {
    fn embed(&self, _: &str) -> CortexResult<Vec<f32>> { Ok(vec![0.0; 1024]) }
    fn embed_batch(&self, texts: &[String]) -> CortexResult<Vec<Vec<f32>>> { Ok(texts.iter().map(|_| vec![0.0; 1024]).collect()) }
...
impl ICausalStorage for MockCausalStorage {
    fn add_edge(&self, _: &CausalEdge) -> CortexResult<()> { Ok(()) }
    fn get_edges(&self, _: &str) -> CortexResult<Vec<CausalEdge>> { Ok(vec![]) }
    fn remove_edge(&self, _: &str, _: &str) -> CortexResult<()> { Ok(()) }
    fn update_strength(&self, _: &str, _: &str, _: f64) -> CortexResult<()> { Ok(()) }
    fn add_evidence(&self, _: &str, _: &str, _: &CausalEvidence) -> CortexResult<()> { Ok(()) }
    fn has_cycle(&self, _: &str, _: &str) -> CortexResult<bool> { Ok(false) }
    fn edge_count(&self) -> CortexResult<usize> { Ok(0) }
    fn node_count(&self) -> CortexResult<usize> { Ok(0) }
    fn remove_orphaned_edges(&self) -> CortexResult<usize> { Ok(0) }
}

struct MockRetriever;
impl IRetriever for MockRetriever {
    fn retrieve(&self, _: &RetrievalContext, _: usize) -> CortexResult<Vec<CompressedMemory>> { Ok(vec![]) }
}
...
impl IConsolidator for MockConsolidator {
    fn consolidate(&self, _: &[BaseMemory]) -> CortexResult<ConsolidationResult> {
        Ok(ConsolidationResult { created: vec![], archived: vec![], metrics: ConsolidationMetrics { precision: 1.0, compression_ratio: 1.0, lift: 0.0, stability: 1.0 } })
    }
}

struct MockDecayEngine;
impl IDecayEngine for MockDecayEngine {
    fn calculate(&self, _: &BaseMemory) -> CortexResult<f64> { Ok(0.95) }
}

struct MockValidator;
impl IValidator for MockValidator {
    fn validate(&self, _: &BaseMemory) -> CortexResult<ValidationResult> {
        Ok(ValidationResult { memory_id: String::new(), dimension_scores: DimensionScores { citation: 1.0, temporal: 1.0, contradiction: 1.0, pattern_alignment: 1.0 }, overall_score: 1.0, healing_actions: vec![], passed: true })
    }
}
...
impl ICompressor for MockCompressor {
    fn compress(&self, _: &BaseMemory, _: u8) -> CortexResult<CompressedMemory> {
        Ok(CompressedMemory { memory_id: String::new(), memory_type: cortex_core::MemoryType::Core, importance: cortex_core::Importance::Normal, level: 0, text: String::new(), token_count: 0, relevance_score: 0.0 })
    }
    fn compress_to_fit(&self, m: &BaseMemory, _: usize) -> CortexResult<CompressedMemory> { self.compress(m, 0) }
    fn compress_batch_to_fit(&self, _: &[BaseMemory], _: usize) -> CortexResult<Vec<CompressedMemory>> { Ok(vec![]) }
}

struct MockSanitizer;
impl ISanitizer for MockSanitizer {
    fn sanitize(&self, text: &str) -> CortexResult<SanitizedText> {
        Ok(SanitizedText { text: text.to_string(), redactions: vec![] })
    }
}
...
impl IPredictor for MockPredictor {
    fn predict(&self, _: &PredictionSignals) -> CortexResult<PredictionResult> {
        Ok(PredictionResult { memory_ids: vec![], signals: vec![], confidence: 0.0 })
    }
}
...
impl ILearner for MockLearner {
    fn analyze(&self, _: &Correction) -> CortexResult<LearningResult> {
        Ok(LearningResult { category: "test".into(), principle: None, memory_created: None })
    }
}
...
impl IHealthReporter for MockHealthReporter {
    fn report(&self) -> CortexResult<HealthReport> {
        Ok(HealthReport { overall_status: HealthStatus::Healthy, subsystems: vec![], metrics: HealthMetrics { total_memories: 0, active_memories: 0, archived_memories: 0, average_confidence: 0.0, db_size_bytes: 0, embedding_cache_hit_rate: 0.0 } })
    }
}
