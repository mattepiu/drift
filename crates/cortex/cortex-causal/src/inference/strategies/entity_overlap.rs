...
fn file_overlap(a: &BaseMemory, b: &BaseMemory) -> f64 {
    ...
    let set_a: std::collections::HashSet<&str> =
        a.linked_files.iter().map(|f| f.file_path.as_str()).collect();
    let set_b: std::collections::HashSet<&str> =
        b.linked_files.iter().map(|f| f.file_path.as_str()).collect();
    jaccard(&set_a, &set_b)
}
...
fn function_overlap(a: &BaseMemory, b: &BaseMemory) -> f64 {
    ...
    let set_a: std::collections::HashSet<&str> =
        a.linked_functions.iter().map(|f| f.function_name.as_str()).collect();
    let set_b: std::collections::HashSet<&str> =
        b.linked_functions.iter().map(|f| f.function_name.as_str()).collect();
    jaccard(&set_a, &set_b)
}
...
fn pattern_overlap(a: &BaseMemory, b: &BaseMemory) -> f64 {
    ...
    let set_a: std::collections::HashSet<&str> =
        a.linked_patterns.iter().map(|p| p.pattern_name.as_str()).collect();
    let set_b: std::collections::HashSet<&str> =
        b.linked_patterns.iter().map(|p| p.pattern_name.as_str()).collect();
    jaccard(&set_a, &set_b)
}
