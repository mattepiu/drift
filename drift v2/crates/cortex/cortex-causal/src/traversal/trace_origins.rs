//! Backward traversal: "what caused this?" — follows incoming edges.
...
pub fn trace(graph: &IndexedGraph, memory_id: &str, config: &TraversalConfig) -> TraversalResult {
    ...
    while let Some((current, depth, path_strength)) = queue.pop_front() {
        ...
        for neighbor in graph.graph.neighbors_directed(current, Direction::Incoming) {
            ...
            // Check edge strength.
            if let Some(edge_idx) = graph.graph.find_edge(neighbor, current) {
